...
function positivMod(n, mod) {
  ...
  class drawing {
    ...
    drawHero(heroInstance) {
      ...
      heroBody.closePath();
      // ===== MUSIC DASH: color personalitzable =====
      const c = (window.MusicDash && window.MusicDash.cubeRGB) || [254, 1, 154];
      this.drawSquareNeonStyle(heroBody, c[0], c[1], c[2]);
      // Dibuixar ulls estil Geometry Dash sobre el cub
      const cx = (this.gridAbscissa(heroInstance.body.vertices[0].x - this.heroCenterXPosition) +
                  this.gridAbscissa(heroInstance.body.vertices[2].x - this.heroCenterXPosition)) / 2;
      const cy = (this.gridOrdinate(heroInstance.body.vertices[0].y + this.heroAjustYPosition) +
                  this.gridOrdinate(heroInstance.body.vertices[2].y + this.heroAjustYPosition)) / 2;
      const eyeSize = this.unity * 0.18;
      const eyeOffsetX = this.unity * 0.22;
      const eyeOffsetY = this.unity * 0.05;
      this.ctx.save();
      this.ctx.fillStyle = "#ffffff";
      this.ctx.fillRect(cx - eyeOffsetX - eyeSize/2, cy - eyeOffsetY - eyeSize/2, eyeSize, eyeSize);
      this.ctx.fillRect(cx + eyeOffsetX - eyeSize/2, cy - eyeOffsetY - eyeSize/2, eyeSize, eyeSize);
      this.ctx.fillStyle = "#1a1a1a";
      const pup = eyeSize * 0.5;
      this.ctx.fillRect(cx - eyeOffsetX - pup/2, cy - eyeOffsetY - pup/2, pup, pup);
      this.ctx.fillRect(cx + eyeOffsetX - pup/2, cy - eyeOffsetY - pup/2, pup, pup);
      this.ctx.restore();
    }
    ...
    drawBackGround() {
      ...
      for (let i = 0; i < 40; i++) {
        ...
        const y = (i * 53) % (h * 0.65);
        const r = (i % 3) + 1;
        this.ctxBack.globalAlpha = 0.3 + ((i * 7) % 5) / 10;
        this.ctxBack.fillRect(x, y, r, r);
      }
  ...
  function restart(parameters) {
    ...
    // ===== MUSIC DASH: NO resetear el quiz al respawnar desde checkpoint.
    // El state.index se mantiene y al volver al punto aparece la misma pregunta.
  ...
  function game() {
    ...
    if (!heroInstance.isDead && !heroInstance.haveFinished) {
      // ===== MUSIC DASH: hook de preguntas =====
      if (window.MusicQuiz) {
        const paused = window.MusicQuiz.tick(
          heroInstance.body.center.x,
          heroInstance.isJumping,
          function () {
            // onResume: reanuda el loop, resetea tiempo para no hacer saltos bruscos
            frameTimeDiff.lastTime = Date.now();
            try { drawingInstance.sound.backGroundMusic.play(); } catch (e) {}
            requestAnimationFrame(game);
          },
          function () {
            // onDie: mata al héroe
            heroInstance.isDead = true;
            heroInstance.setDeathParticle();
            deathFinish();
          }
        );
        if (paused) {
          try { drawingInstance.sound.backGroundMusic.pause(); } catch (e) {}
          return; // no sigas el frame, el overlay llamará a onResume cuando acierte
        }
      }
      // ===== FIN hook =====
  ...
  // ===== MUSIC DASH: constructor procedural con patrones variados =====
  // Cada patrón tiene un nivel de dificultad (1-5). Cada nivel del juego
  // define rango de dificultad permitido Y cómo evoluciona a lo largo del
  // recorrido (empieza suave, va escalando).
  //
  // Tipos de elementos dentro de un patrón:
  //   { t: "spike", dx }                 -> pincho en dx
  //   { t: "bump",  dx, h, len }         -> plataforma elevada len bloques, altura h
  //   { t: "pit",   dx, w }              -> hueco de ancho w (sin suelo)
  //   { t: "pillar",dx, h }              -> columna vertical de pinchos hacia abajo colgando
  // Tipos soportados:
  //   spike: pincho triangular en suelo
  //   bump:  plataforma elevada (h=altura sobre suelo, len=ancho)  -> cortos = muros, largos = plataformeo
  //   pit:   hueco en el suelo (w = ancho de columnas sin suelo)
  //   (combinando bump + pit se consiguen plataformas flotantes)
  const MD_PATTERNS = [
    // ================= Dificultad 1 (suave) =================
    { diff: 1, len: 14, items: [] }, // tramo llano de respiro
    { diff: 1, len: 14, items: [{ t: "spike", dx: 7 }] },
    { diff: 1, len: 16, items: [{ t: "spike", dx: 9 }] },
    { diff: 1, len: 14, items: [{ t: "bump", dx: 6, h: 1, len: 2 }] },                     // bump bajo
    { diff: 1, len: 14, items: [{ t: "pit",  dx: 7, w: 2 }] },                              // pit pequeño

    // ================= Dificultad 2 (moderada) =================
    { diff: 2, len: 14, items: [{ t: "spike", dx: 5 }, { t: "spike", dx: 10 }] },           // 2 pinchos separados
    { diff: 2, len: 12, items: [{ t: "spike", dx: 4 }, { t: "spike", dx: 5 }] },            // doble pegado
    { diff: 2, len: 14, items: [{ t: "bump", dx: 5, h: 1, len: 3 }] },                     // bump largo bajo
    { diff: 2, len: 14, items: [{ t: "bump", dx: 5, h: 2, len: 2 }] },                     // muro alto corto
    { diff: 2, len: 14, items: [{ t: "pit",  dx: 6, w: 3 }] },                              // pit mediano
    { diff: 2, len: 16, items: [                                                            // stairs 2 pasos
      { t: "bump", dx: 4, h: 1, len: 2 },
      { t: "bump", dx: 8, h: 2, len: 2 }
    ]},
    { diff: 2, len: 16, items: [                                                            // plataforma flotante sencilla
      { t: "pit",  dx: 5, w: 3 },
      { t: "bump", dx: 6, h: 1, len: 2 }
    ]},

    // ================= Dificultad 3 (media) =================
    { diff: 3, len: 16, items: [{ t: "spike", dx: 4 }, { t: "spike", dx: 5 }, { t: "spike", dx: 11 }] },
    { diff: 3, len: 14, items: [{ t: "bump", dx: 5, h: 2, len: 3 }, { t: "spike", dx: 11 }] },
    { diff: 3, len: 14, items: [{ t: "spike", dx: 4 }, { t: "bump", dx: 7, h: 1, len: 2 }] },
    { diff: 3, len: 18, items: [                                                            // escalera 3 pasos
      { t: "bump", dx: 3,  h: 1, len: 2 },
      { t: "bump", dx: 7,  h: 2, len: 2 },
      { t: "bump", dx: 11, h: 3, len: 2 }
    ]},
    { diff: 3, len: 16, items: [{ t: "pit", dx: 4, w: 3 }, { t: "spike", dx: 11 }] },       // pit + pincho
    { diff: 3, len: 18, items: [
      { t: "pit",  dx: 4, w: 3 },
      { t: "bump", dx: 4, h: 1, len: 2 },
      { t: "bump", dx: 8, h: 2, len: 2 }
    ]},
    { diff: 3, len: 16, items: [{ t: "bump", dx: 4, h: 2, len: 2 }, { t: "bump", dx: 9, h: 2, len: 2 }] }, // dos muros

    // ================= Dificultad 4 (difícil) =================
    { diff: 4, len: 16, items: [{ t: "spike", dx: 4 }, { t: "spike", dx: 5 }, { t: "spike", dx: 6 }] }, // triple pegado
    { diff: 4, len: 18, items: [                                                             // combo bump-bump-spike
      { t: "bump", dx: 4, h: 1, len: 2 },
      { t: "bump", dx: 9, h: 2, len: 2 },
      { t: "spike", dx: 14 }
    ]},
    { diff: 4, len: 14, items: [{ t: "spike", dx: 3 }, { t: "spike", dx: 6 }, { t: "spike", dx: 9 }] },
    { diff: 4, len: 18, items: [                                                             // muro alto + doble pincho
      { t: "bump", dx: 4, h: 2, len: 3 },
      { t: "spike", dx: 11 },
      { t: "spike", dx: 12 }
    ]},
    { diff: 4, len: 18, items: [{ t: "pit", dx: 5, w: 3 }, { t: "spike", dx: 12 }] },        // pit + pincho
    { diff: 4, len: 18, items: [                                                             // plataforma flotante + pincho detrás
      { t: "pit",  dx: 4, w: 3 },
      { t: "bump", dx: 5, h: 1, len: 2 },
      { t: "spike", dx: 12 }
    ]},
    { diff: 4, len: 18, items: [                                                             // escalera ascendente con pincho en cima
      { t: "bump", dx: 3, h: 1, len: 2 },
      { t: "bump", dx: 6, h: 2, len: 2 },
      { t: "bump", dx: 9, h: 3, len: 2 },
      { t: "spike", dx: 14 }
    ]},

    // ================= Dificultad 5 (extrema) =================
    { diff: 5, len: 18, items: [{ t: "spike", dx: 3 }, { t: "spike", dx: 4 }, { t: "spike", dx: 8 }, { t: "spike", dx: 9 }] },
    { diff: 5, len: 18, items: [
      { t: "bump", dx: 3, h: 1, len: 2 },
      { t: "spike", dx: 7 },
      { t: "spike", dx: 8 },
      { t: "bump", dx: 12, h: 2, len: 2 }
    ]},
    { diff: 5, len: 16, items: [{ t: "spike", dx: 3 }, { t: "spike", dx: 4 }, { t: "spike", dx: 5 }, { t: "spike", dx: 10 }, { t: "spike", dx: 11 }] },
    { diff: 5, len: 20, items: [                                                             // pit-flotante-pit-pincho
      { t: "pit",  dx: 3, w: 2 },
      { t: "bump", dx: 6, h: 1, len: 2 },
      { t: "pit",  dx: 9, w: 2 },
      { t: "spike", dx: 14 }
    ]},
    { diff: 5, len: 20, items: [                                                             // muro-pincho-muro-pincho
      { t: "bump", dx: 3, h: 2, len: 2 },
      { t: "spike", dx: 8 },
      { t: "bump", dx: 11, h: 2, len: 2 },
      { t: "spike", dx: 16 }
    ]},
    { diff: 5, len: 20, items: [                                                             // escalera + pit al final
      { t: "bump", dx: 3,  h: 1, len: 2 },
      { t: "bump", dx: 6,  h: 2, len: 2 },
      { t: "bump", dx: 9,  h: 3, len: 2 },
      { t: "pit",  dx: 13, w: 3 }
    ...
    // ===== Patrons addicionals d'alta dificultat =====
    // 5+ pinchos amb gaps mínims (dif 4)
    { diff: 4, len: 16, items: [
      { t: "spike", dx: 3 }, { t: "spike", dx: 5 },
      { t: "spike", dx: 7 }, { t: "spike", dx: 9 },
      { t: "spike", dx: 11 }
    ]},
    // Salts continus per damunt de bumps alts (dif 4)
    { diff: 4, len: 18, items: [
      { t: "bump", dx: 3, h: 2, len: 1 },
      { t: "bump", dx: 6, h: 2, len: 1 },
      { t: "bump", dx: 9, h: 2, len: 1 },
      { t: "bump", dx: 12, h: 2, len: 1 }
    ]},
    // Pit + bump-flotant + pincho a sobre (dif 5)
    { diff: 5, len: 18, items: [
      { t: "pit",  dx: 4, w: 3 },
      { t: "bump", dx: 5, h: 1, len: 2 },
      { t: "spike", dx: 12 },
    ...
    // Pinchos + bump + pinchos (dif 5)
    { diff: 5, len: 20, items: [
      { t: "spike", dx: 3 }, { t: "spike", dx: 4 },
      { t: "bump",  dx: 7, h: 2, len: 2 },
      { t: "spike", dx: 13 }, { t: "spike", dx: 14 }
    ]},
    // Pinchos quàdruples seguits + pit (dif 5)
    { diff: 5, len: 20, items: [
      { t: "spike", dx: 3 }, { t: "spike", dx: 4 },
      { t: "spike", dx: 5 }, { t: "spike", dx: 6 },
      { t: "pit", dx: 11, w: 3 }
    ]},
    // Doble escalera ascendent → descendent (dif 5)
    { diff: 5, len: 22, items: [
      { t: "bump", dx: 3,  h: 1, len: 2 },
      { t: "bump", dx: 6,  h: 2, len: 2 },
      { t: "bump", dx: 9,  h: 3, len: 2 },
      { t: "bump", dx: 13, h: 2, len: 2 },
      { t: "bump", dx: 16, h: 1, len: 2 }
    ]}
  ];
  ...
  function buildMusicDashLevel(gridInstance, heroInstance, lvlIdx) {
    ...
    const QUIZ_DIST = 200;
    const TOTAL_Q = 10;
    const SAFE_ZONE_LEN = 24;
    const LENGTH = QUIZ_DIST * (TOTAL_Q + 1);

    for (let k = 0; k < 40; k++) gridInstance.addPlatform(k, 4);
    let x = 40;

    // PRNG amb seed fix per nivell: mateix nivell = mateix layout sempre
    let seed = (lvlIdx + 1) * 1337;
    const rng = () => { seed = (seed * 9301 + 49297) % 233280; return seed / 233280; };
    const rand = (a, b) => a + Math.floor(rng() * (b - a + 1));
    const pick = arr => arr[Math.floor(rng() * arr.length)];

    while (x < LENGTH) {
      // Zona segura alrededor del próximo punto de pregunta
      const nextQuizX = Math.ceil(x / QUIZ_DIST) * QUIZ_DIST;
      const isNearQuiz = (nextQuizX - x) < SAFE_ZONE_LEN + 6;
      if (isNearQuiz && nextQuizX < LENGTH - 50) {
        const end = nextQuizX + SAFE_ZONE_LEN;
        for (let k = x; k < end; k++) gridInstance.addPlatform(k, 4);
        x = end;
        continue;
      }

      // Dificultad actual interpolada según progreso en el nivel
      const progress = x / LENGTH; // 0..1
      const currentDiff = cfg.diffStart + (cfg.diffEnd - cfg.diffStart) * progress;
      const minAllowed = Math.max(1, Math.floor(currentDiff - 1));
      const maxAllowed = Math.min(5, Math.ceil(currentDiff));

      if (rng() < cfg.flatChance) {
        const flatLen = rand(8, 14);
        for (let k = 0; k < flatLen; k++) gridInstance.addPlatform(x + k, 4);
        x += flatLen;
        continue;
      }

      // Elegir patrón de esa franja de dificultad
      const candidates = MD_PATTERNS.filter(p => p.diff >= minAllowed && p.diff <= maxAllowed);
      const pattern = pick(candidates);

      // Dibujar el patrón: suelo base + items
      for (let k = 0; k < pattern.len; k++) {
        // ¿Hay pit en esta columna?
        const pit = pattern.items.find(it => it.t === "pit" && k >= it.dx && k < it.dx + (it.w || 1));
        if (!pit) gridInstance.addPlatform(x + k, 4);
      }
      for (const it of pattern.items) {
        if (it.t === "spike") {
          gridInstance.addPeak(x + it.dx, 5, "up");
        } else if (it.t === "bump") {
          const h = it.h || 1;
          const len = it.len || 2;
          for (let k = 0; k < len; k++) {
            gridInstance.addPlatform(x + it.dx + k, 4 + h);
          }
        }
      }
      x += pattern.len;
    ...
    // Tramo final llano
    for (let k = 0; k < 60; k++) gridInstance.addPlatform(x + k, 4);
    const endingInstance = new ending(x + 70);
    gridInstance.addEnding(endingInstance);
  }
  // ===== FIN MUSIC DASH builder =====

  function level1(gridInstance, heroInstance) {
    // ===== MUSIC DASH: level procedural basado en nivel elegido =====
    const lvlIdx = (window.MusicDash && window.MusicDash.currentLevel) || 0;
    return buildMusicDashLevel(gridInstance, heroInstance, lvlIdx);
    // ===== Original level1 descartado (conservado abajo como referencia) =====
    gridInstance.grid = [];
